...
- **Bug-proof test**: `test_regression_BUG_responds_to_previous_message_foreign_result_does_not_end_turn` (+ `..._foreign_messages_not_rendered`) in `bots/coder/tests/test_coder_main.py` — fails first, passes after fix
- **Solution**: in `_stream_response`, compute each message's origin (`session_id`) and ignore foreign-origin messages: a foreign `ResultMessage` no longer ends the turn (logged instead), foreign assistant/tool output is not rendered into the user's card, and `session.session_id` is only adopted from the session's own messages. The turn now ends only on **its own** session's `ResultMessage`, which makes the one-behind desync impossible regardless of what else shares the stream.

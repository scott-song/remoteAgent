...
| F-13 | F-1's sibling path is still racy: `_handle_attachments` invokes the callback without waiting on this sender's other in-flight receives, so image-then-**captioned-post** can still attach the wrong set | Important | `core/src/core/feishu_client.py:387-390` vs the guard at `:257-266` | fixed |
| F-14 | The new wait has no timeout and no fallback delivery, and waits jobs **sequentially** — a stalled receive silently withholds that sender's messages | Important | `core/src/core/feishu_client.py:295-306`; lark default HTTP timeout 30 s (`lark_oapi/core/model/config.py:13-14`) × `UPDATE_MAX_RETRIES=2` | fixed |
| F-15 | `too_large` always logs `size=10485761` (`max_bytes + 1`) — a floor rendered as an exact size, and the two new tests pin `11534336`, which production cannot produce | Suggestion | `core/src/core/feishu_client.py:618-622`; `core/tests/test_feishu_client.py:1067, 850, 921` | fixed |
...
| F-17 | `test_a_failed_receive_still_delivers_the_text` does not deterministically exercise `_deliver_after`'s `except` branch | Suggestion | `core/tests/test_feishu_client.py:1013-1032` | fixed |
| F-18 | The tester's E2E evidence now predates the code: `report_at_head=false`, `code_moved=5`, and `role-dev` edited `test_e2e_image_input.py` itself in this batch | Important | `sdlc-review-facts` (`report_sha=9e0ce9f`, `head_sha=ad8227f`); `git diff 3211aa8..HEAD -- bots/coder/tests/test_e2e_image_input.py` | fixed |
...
Recorded per `playbooks/shared/finding-disposition.md`. The user chose **fix F-13, F-14, F-18 and the
anchor drift, plus F-15 and F-17**, on 2026-08-26. F-16 carried.

| ID | Status | How |
|---|---|---|
| F-13 | **fixed** | Receives serialize per `(sender, chat)` via an asyncio lock on the bot loop, so a captioned post cannot settle before an earlier paste. Proven: making the lock per-call instead of per-key fails `int_AC_2_a_captioned_post_waits_for_an_earlier_paste` on `assert len(held) == 2`. |
| F-14 | **fixed** | You were right that the round-1 fix traded a visible wrong answer for silence. The wait is now bounded by `DELIVERY_GRACE_SECONDS` (3s) and the jobs are awaited **concurrently**; on timeout the text is delivered without its image. Proven: `timeout=None` fails both new tests, including one asserting six stuck jobs cannot serialize into ~1.8s. |
| F-15 | **fixed** | `too_large` renders `size>=N`, since the bounded read makes the figure a floor. Both tests now use `IMAGE_MAX_BYTES + 1` instead of a value production cannot emit, and one asserts the exact-size form is **absent**. |
| F-17 | **fixed** | The failed-receive test is gated on an `asyncio.Event` and now asserts `cb.call_count == 0` before release, so the branch it names is provably executed. |
| F-18 | **fixed** | Both levels re-run at `94818ef` — the tree containing every fix — and the report re-stamped: level 1 16/16, level 2 418/418, chain 4/4 fresh. |
| plan→design drift | **fixed** | Design re-anchored to `h1:e0e0bba14e61`; `sdlc-freshness` reports 4/4 fresh. |
| F-16 | **carried, open** | The `NamedTuple` for `download_resource`'s return. Agreed as a real smell after two shape changes; not blocking. |
| F-6 … F-12 | **carried, open** | Unchanged from round 1. |

### A bug the round-2 work surfaced that no review round had found

Writing F-13's test forced the attachment arithmetic into the open: a captioned post attached its image
**twice**. `_handle_attachments` stored it *and* passed it inline, so the consumer's `held + inline`
counted it double — double vision-token cost and two of the five cap slots for one image. Every
existing assertion at every layer checked the callback's argument rather than the prompt, so AC-2 was
green throughout. The store is now the single drain; the callback carries nothing inline. Two e2e tests
count the prompt directly, and restoring the old behaviour fails both.

Gates at the tree these landed on: suite **418/418**, `make typecheck` clean, `ruff check`/`format`
clean, freshness **4/4 fresh**.

...
def _make_agent(name="proj1", project_dir="/tmp/proj1", model="sonnet",
                permission_mode="acceptEdits", github_url=None,
                feishu_chat_ids=None):
    """Create a lightweight agent-like object."""
...
@pytest.fixture
def bot():
    with patch('bot.main.AgentRegistry') as mock_registry_cls, \
        ...
        b.agents = mock_registry_cls.return_value
...
class TestHandleCommand:
    ...
    def test_projects_lists_agents(self, bot):
        agents = [_make_agent("p1"), _make_agent("p2")]
        bot.agents.list_agents.return_value = agents
        bot.agents.get_by_chat_id.return_value = None
        bot._user_agents = {}
    ...
    def test_project_with_valid_name(self, bot):
        agent = _make_agent("proj1")
        bot.agents.get.return_value = agent
        bot._handle_command("/project proj1", "chat1", "user1", "msg1")
        bot.feishu.reply.assert_called_once()
        reply_text = bot.feishu.reply.call_args[0][1]
        assert "proj1" in reply_text
        assert bot._user_agents["user1"] == "proj1"

    def test_project_with_invalid_name(self, bot):
        bot.agents.get.return_value = None
        bot.agents.list_agents.return_value = [_make_agent("p1")]
    ...
    def test_mode_valid(self, bot):
        ...
        bot.agents.get_by_chat_id.return_value = _make_agent("proj1")
...
class TestResolveAgent:
    def test_returns_agent_from_chat_binding(self, bot):
        agent = _make_agent("bound-proj")
        bot.agents.get_by_chat_id.return_value = agent
        assert bot._resolve_agent("user1", "chat1") == "bound-proj"

    def test_falls_back_to_user_last_selected(self, bot):
        bot.agents.get_by_chat_id.return_value = None
        bot._user_agents = {"user1": "user-proj"}
        bot.agents.get.return_value = _make_agent("user-proj")
        assert bot._resolve_agent("user1", "chat1") == "user-proj"

    def test_falls_back_to_first_agent(self, bot):
        bot.agents.get_by_chat_id.return_value = None
        bot._user_agents = {}
        bot.agents.get.return_value = None
        bot.agents.list_agents.return_value = [_make_agent("first")]
        assert bot._resolve_agent("user1", "chat1") == "first"

    def test_returns_empty_when_no_agents(self, bot):
        bot.agents.get_by_chat_id.return_value = None
        bot._user_agents = {}
        bot.agents.get.return_value = None
        bot.agents.list_agents.return_value = []
        assert bot._resolve_agent("user1", "chat1") == ""
...
class TestProjectManagement:
    def test_addproject_valid(self, bot):
        bot.agents.add.return_value = _make_agent("newproj", "/tmp/newproj")
        bot._handle_command("/addproject newproj /tmp/newproj", "chat1", "user1", "msg1")
        bot.agents.add.assert_called_once_with(
    ...
    def test_addproject_with_bind_and_github(self, bot):
        bot.agents.add.return_value = _make_agent("proj", "/tmp/proj", github_url="https://github.com/u/r")
        bot._handle_command(
    ...
    def test_addproject_duplicate(self, bot):
        bot.agents.add.side_effect = ValueError("duplicate")
    ...
    def test_removeproject_valid(self, bot):
        bot.agents.remove.return_value = True
    ...
    def test_removeproject_not_found(self, bot):
        bot.agents.remove.return_value = False
    ...
    def test_bind_with_name(self, bot):
        bot.agents.bind_chat.return_value = None
        bot.agents.get.return_value = _make_agent("proj1")
        bot._handle_command("/bind proj1", "chat1", "user1", "msg1")
        bot.agents.bind_chat.assert_called_once_with("proj1", "chat1")
        reply_text = bot.feishu.reply.call_args[0][1]
        assert "Bound" in reply_text
    ...
    def test_unbind(self, bot):
        bot.agents.unbind_chat.return_value = "proj1"
    ...
    def test_unbind_not_bound(self, bot):
        bot.agents.unbind_chat.return_value = None
        bot._handle_command("/unbind", "chat1", "user1", "msg1")
        reply_text = bot.feishu.reply.call_args[0][1]
        assert "Not bound" in reply_text
...
class TestHandlePrompt:
    @pytest.mark.asyncio
    async def test_no_agent_sends_error(self, bot):
        bot.agents.get_by_chat_id.return_value = None
        bot._user_agents = {}
        bot.agents.get.return_value = None
        bot.agents.list_agents.return_value = []
        ...
        # _resolve_agent returns "" and agents.get("") returns None
    ...
    async def test_git_sync_called_when_github_url_set(self, bot):
        agent = _make_agent("proj1", github_url="https://github.com/u/r")
        bot.agents.get_by_chat_id.return_value = agent
        bot.agents.get.return_value = agent
        bot.sessions.get.return_value = None
        bot.sessions.cleanup_stale = AsyncMock()
        ...
        with patch('bot.main.create_claude_client', return_value=mock_client), \
            ...
            mock_sync.assert_called_once_with(agent.project_dir, "https://github.com/u/r")
